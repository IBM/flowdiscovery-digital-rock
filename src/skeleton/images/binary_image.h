/**
 * \file src/skeleton/images/binary_image.h
 * \brief This file implements a data structure to represente a binary image.
 *
...
#ifndef SRC_SKELETON_IMAGES_BINARY_IMAGE_H_
#define SRC_SKELETON_IMAGES_BINARY_IMAGE_H_
...
class BinaryImage {
 public:
  ...
  using my_type = BinaryImage<point_type>;
  ...
  using ObjectPoints = std::vector<bool>;
 ...
 public:
  ...
  BinaryImage(const std::vector<bool> &points_values, array sizes)
    : points_(points_values),
      sizes_(sizes) {
    size_ = 1;
    for (typename array::size_type index = 0; index != sizes_.size(); ++index)
      size_ *= static_cast<ObjectPoints::size_type>(sizes_[index]);
  }
  ...
  ObjectPoints::size_type GetTotalSize() const {
    return size_;
  }
  ...
  bool IsContourPoint(const point_type &point) const noexcept {
    if (IsOffLimitsPoint(point))
      return false;
    auto index = ConvertToLinearIndex(point);
    return points_[index] == Background;
  }
  ...
  bool IsObjectPoint(const point_type &point) const noexcept {
    if (IsOffLimitsPoint(point))
      return false;
    auto index = ConvertToLinearIndex(point);
    return points_[index] == Foreground;
  }
...
#endif  // SRC_SKELETON_IMAGES_BINARY_IMAGE_H_
